...
class TemplateElementService:
    ...
    async def apply_patches(self, template_id: uuid.UUID, patches: Sequence[AnyPatch]) -> None:
        if not patches:
            return

        logger.info(
            "template_elements.apply_patches.start",
        ...
        for patch in patches:
            if patch.action == PatchAction.DELETE:
                ...
                logger.error(
                    "template_elements.apply_patches.unknown_action",
                    action=str(patch.action),
                )
                raise TemplatePatchException(f"Неизвестное действие патча: {patch.action}")

        created_ids = {temp_id_map.get(p.payload.id, p.payload.id) for p in create_patches}
        updated_ids = set(updates_map.keys())
        deleted_ids = set(ids_to_delete)
        conflict_ids = (created_ids & updated_ids) | (created_ids & deleted_ids)

        if conflict_ids:
            logger.warning(
                "template_elements.apply_patches.conflict_ids",
                template_id=str(template_id),
                conflict_ids=[str(i) for i in conflict_ids],
            )
            raise TemplatePatchException(f"Конфликт действий для элементов: {conflict_ids}")
        ...
        if ids_to_delete:
            logger.info(
                "template_elements.apply_patches.delete",
                template_id=str(template_id),
        ...
        if elements_to_create:
            logger.info(
                "template_elements.apply_patches.create",
                template_id=str(template_id),
                create_count=len(elements_to_create),
            )
            await self.repository.bulk_create_raw(elements_to_create)

        if updates_map:
            logger.info(
                "template_elements.apply_patches.update",
                template_id=str(template_id),
                update_count=len(updates_map),
            )
            await self.repository.bulk_update_from_map(template_id, updates_map)

        logger.info(
            "template_elements.apply_patches.ok",
            template_id=str(template_id),
        )
    ...
    def build_tree(self, db_elements: Sequence[TemplateElement]) -> list[TemplateElementResponse]:
        ...
        nodes_map: dict[uuid.UUID, TemplateElementResponse] = {
            el.id: TemplateElementResponse.from_db(el) for el in db_elements
        }
