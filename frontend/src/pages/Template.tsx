...
function AddMenu({
  ...
  const MenuItem = ({
    ...
    <button
      type="button"
      className="w-full text-left px-4 py-3 rounded-xl hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors"
      onClick={() => {
        onClick();
        setOpen(false);
      }}
  ...
  return (
    <div className="relative">
      <button
        ...
        className={cx(
          "px-5 py-3 text-sm font-semibold rounded-xl border",
      ...
      {open && (
        <div
          ref={popupRef}
          className={cx(
            "absolute right-0 mt-2 z-[120] w-80",
        ...
        >
          <div className="p-2 space-y-1">
            ...
            <MenuItem
              title="Вопрос + Ответ"
              description="Создать пару question/answer"
              onClick={onAddQA}
            />
            <MenuItem
              title="Картинка"
              description="Загрузка файла и создание блока"
              onClick={onAddImage}
            />
            <MenuItem
              title="Группа"
              description="Контейнер для вложенных блоков"
              onClick={onAddGroup}
            />
          </div>
        </div>
...
export function TemplatePage({ templateId, initialData }: TemplatePageProps) {
  ...
  const canEdit = !!template._links.update && template.isDraft;
  ...
  const metaBaselineRef = useRef({
    name: template.name,
    maxScore: normalizeTotalPoints(template.maxScore),
  });
  ...
  const metaDirty =
    name !== metaBaselineRef.current.name ||
    totalPoints !== metaBaselineRef.current.maxScore;
  const hasUnsaved = editor.isDirty || metaDirty;
  ...
  useEffect(() => {
    if (!hasUnsaved) {
      ...
      metaBaselineRef.current = nextMeta;
      setName(nextMeta.name);
      setTotalPoints(nextMeta.maxScore);
    }
  }, [template.name, template.maxScore, hasUnsaved]);

  const availableQuestions = useQuestions(editor.elements);
  const globalScoring = useGlobalScoring(editor.elements, totalPoints);

  const uploadHref = template._links.upload_image?.href;
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [pendingImage, setPendingImage] = useState<PendingImageAction | null>(
    null,
  );
  ...
  const uploadMutation = useMutation({
    mutationFn: async (file: File) => {
      if (!uploadHref)
        throw new Error("Нет HAL-ссылки для загрузки изображения");
      return templateApi.uploadImage(uploadHref, file);
    },
    onError: (err) => {
      const message =
  ...
  const openCreateImage = useCallback(() => {
    if (!canEdit) return;
    if (!uploadHref) {
      toast.error("Загрузка изображений недоступна (нет upload_image в HAL)");
      return;
    }
    setPendingImage({ kind: "create" });
  ...
  const handleFilePicked = useCallback(
    async (file: File) => {
      ...
      if (pendingImage.kind === "create") {
        editor.addImageWithMediaKey(mediaKey, file.name);
      } else if (pendingImage.elementId) {
        editor.updateElement(pendingImage.elementId, { mediaKey });
      }
      setPendingImage(null);
    },
    [pendingImage, uploadMutation, editor],
  );
  ...
  const saveMutation = useMutation({
    mutationFn: async ({ mode }: { mode: SaveMode }) => {
      ...
      const body: TemplateUpdateRequest = { name, maxScore: totalPoints };
      if (patches.length > 0) body.elements = { patches };
    ...
    onSuccess: async (_data, variables) => {
      ...
      metaBaselineRef.current = { name, maxScore: totalPoints };
  ...
  const handleSave = useCallback(
    (mode: SaveMode) => {
      if (mode === "save" && !canEdit) return;
      if (mode === "publish" && !canPublish) return;
      saveMutation.mutate({ mode });
    },
    [saveMutation, canEdit, canPublish],
  );

  useHotkeySave(canEdit && template.isDraft, () => handleSave("save"));
  ...
  return (
    <>
      <Helmet>
        <title>{(template.isDraft ? "Черновик " : "Шаблон ") + name}</title>
      </Helmet>
      ...
      <div className="sticky top-0 z-[80] isolate border-b border-zinc-200/80 dark:border-zinc-800/80 bg-white/80 dark:bg-[#0F0F12]/80 backdrop-blur">
        ...
        <div className="w-full px-6 pb-3 flex flex-wrap items-end gap-4">
          <div className="text-sm text-zinc-600 flex items-center gap-3">
            <span className="text-zinc-500">Всего баллов (шаблон):</span>
            {canEdit ? (
              <input
                type="number"
                min={1}
                ...
                step={1}
                inputMode="numeric"
                className="w-28 h-10 rounded-xl px-3 bg-transparent border border-zinc-300 outline-none font-semibold text-zinc-900 dark:border-zinc-700 dark:text-white"
                value={totalPoints}
                onChange={(e) =>
          ...
          {canEdit && (
            <AddMenu
              ...
              onAddQA={editor.addQuestionAnswer}
          ...
          <button
            ...
            onClick={() => {
              ...
              setName(metaBaselineRef.current.name);
              setTotalPoints(metaBaselineRef.current.maxScore);
              setError(null);
            }}
